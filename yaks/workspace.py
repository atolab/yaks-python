...
class Workspace(object):
    ...
    def get(self, selector, quorum=1, encoding=Encoding.RAW,
        ...
        '''gets the set of tuples  *<path,value>* available in YAKS for which  the path 
        matches the selector, where the selector can be absolute or relative to the workspace. 
      
        If a **quorum** is provided, then **get** will complete succesfully if and only if a number **quorum** 
        of independent and complete storage set exist. Complete storage means a storage that fully covers the selector 
        (i.e. any path matching the selector is covered by the storage).
        This ensures that if there is a  *{ <path,value> }* stored in YAKS for which the *path* matches the selector **s**, 
        then there are at least **quorum** idependent copies of this element stored in YAKS. Of these **quorum** idependent copies, 
    ...
    def subscribe(self, selector, listener=None):
        ...
        The **selector** can be absolute or relative to the workspace. If specified, 
    ...
    def unregister_eval(self, path):
        '''Unregisters an previously registered evaluation function under the give [path].
        The [path] can be absolute or relative to the workspace.'''
    ...
    def eval(self, selector, multiplicity=1, encoding=Encoding.RAW,
        ...
        **path** matches the given **selector**.
        ...
        Note that in such case, the returned *{ <path,value> }* will contain N time each matching path with the different 
